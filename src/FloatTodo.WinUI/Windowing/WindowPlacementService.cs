...
internal sealed class WindowPlacementService(MainWindow window, nint hwnd)
{
    ...
    public void BeginPointerDrag() => _dragMonitors = _monitors.All();
    public void EndPointerDrag() => _dragMonitors = null;
    ...
    {
        var bounds = Bounds;
        var candidate = PixelRect.FromXYWH(cursor.X - anchor.X, cursor.Y - anchor.Y, bounds.Width, bounds.Height);
        ...
        var placement = DockGeometry.CalculateDockPlacement(cursor, anchor, bounds.Width, bounds.Height, monitor, currentSide);
        Move(placement.TargetRect);
    ...
    public DockSide Snap()
    {
        ...
        return MoveDuringDrag(new(cursor.X, cursor.Y), new(cursor.X - bounds.Left, cursor.Y - bounds.Top), DockSide.None);
    }

    public void Move(PixelRect rect) { var current = Bounds; if (current.Left != rect.Left || current.Top != rect.Top) NativeMethods.SetWindowPos(hwnd, 0, rect.Left, rect.Top, 0, 0, 0x15); }
