...
internal sealed class WindowDragController : IDisposable
{
    ...
    private readonly PointerEventHandler _press, _release, _canceled, _captureLost;
    ...
    private void Press(object sender, PointerRoutedEventArgs e)
    {
        ...
        _edges = (point.X < 6 ? 1 : point.X > root.ActualWidth - 6 ? 2 : 0)
            | (point.Y < 6 ? 4 : point.Y > root.ActualHeight - 6 ? 8 : 0);
        ...
        _placement.BeginPointerDrag(); CompositionTarget.Rendering += Frame; e.Handled = true;
    }

    private void Frame(object? sender, object e) => Tick();
    private void Tick()
    {
        ...
        if (_edges == 0) { _side = _placement.MoveDuringDrag(new(p.X, p.Y), _anchor, _side); return; }
    ...
    private void Finish(bool cancel)
    {
        ...
        {
            _dragging = false;
            if (cancel) _window.AppWindow.MoveAndResize(new(_original.Left, _original.Top, _original.Width, _original.Height));
            _dock.EndDrag(cancel ? _originalSide : _side);
        }
        else if (_edges != 0)
        {
            _dock.AlignToEdge();
